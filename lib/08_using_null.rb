...
def dept_staff_counts
  ...
  execute(<<-SQL)
    SELECT d.name, count(distinct t.name)
    FROM teachers t
    RIGHT JOIN depts d ON t.dept_id = d.id
    GROUP BY 1
  SQL
...
def teachers_and_divisions
  ...
  execute(<<-SQL)
    SELECT
      t.name
      , CASE WHEN d.id in (1,2) THEN 'Sci' ELSE 'Art' END as dept
    FROM teachers t
    LEFT JOIN depts d ON t.dept_id = d.id
  SQL
end
...
def teachers_and_divisions_two
  ...
  execute(<<-SQL)
    SELECT
      t.name
      , CASE WHEN d.id in (1,2) THEN 'Sci' WHEN d.id = 3 then 'Art' ELSE 'None' END as dept
    FROM teachers t
    LEFT JOIN depts d ON t.dept_id = d.id
  SQL
end
