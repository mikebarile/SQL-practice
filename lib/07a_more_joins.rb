...
def expensive_tastes
  ...
  execute(<<-SQL)
    WITH ad as (
      SELECT asin, price, count(song) as tot_songs
      FROM albums
    ...
    SELECT
      s.style
      , sum(ad.price) / sum(ad.tot_songs)
    FROM styles s
    JOIN ad on s.album = ad.asin
    GROUP BY 1
    HAVING sum(ad.price) / sum(ad.tot_songs) > 0
    ORDER BY 2 desc
    LIMIT 5
  SQL
end
